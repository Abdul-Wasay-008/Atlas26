...
export const ENCKE_ORBIT_PARAMS: PlanetOrbitParams = {
    periodDays: 1204,
    semiMajorAxis: 14,
    eccentricity: 0.85,
};

const INCLINATION_RADIANS = THREE.MathUtils.degToRad(5);
const ENCKE_SPEED_MULTIPLIER = 1.5;
...
/**
 * Get Encke's Comet orbital position in heliocentric coordinates (Sun at origin).
 * Uses a local Kepler solver with phase-level speed scaling (shorter effective period
 * for mean anomaly only). 5-degree inclination applied per-position.
 */
export function getEnckePosition(date: Date): THREE.Vector3 {
    const { periodDays, semiMajorAxis, eccentricity } = ENCKE_ORBIT_PARAMS;
    const effectivePeriodDays = periodDays / ENCKE_SPEED_MULTIPLIER;
