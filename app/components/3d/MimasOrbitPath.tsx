...
import { getMimasPosition, MIMAS_ORBITAL_PERIOD_HOURS } from "@/app/astronomy/mimasOrbit";
import { MIMAS_ORBIT_COLOR } from "@/app/data/satelliteOrbitColors";
...
/**
 * Mimas Orbit Path Visualizer
 *
 * Renders Mimas's orbit around Saturn when Mimas or Saturn is selected, or all orbits are shown.
 * The path is a closed ellipse centered on Saturn's current position.
 */
export default function MimasOrbitPath() {
    const { selectedId, showAllOrbits } = useSelectionStore();
    const { currentDate } = useTimeManager();

    const showMimasOrbit = selectedId === "mimas" || selectedId === "saturn" || showAllOrbits;
    ...
    const orbitPoints = useMemo(() => {
        ...
        for (let i = 0; i <= NUM_SAMPLES; i++) {
            ...
            points.push(saturnPositionNow.clone().add(mimasRelativeToSaturn));
        }
    ...
    if (!showMimasOrbit || orbitPoints.length === 0) {
        return null;
    }

    const isFocused = selectedId === "mimas";
