...
import { getDioneWorldPosition } from "@/app/astronomy/dioneOrbit";
import { getPlanetOrbitPosition, SATURN_ORBIT_PARAMS } from "@/app/astronomy/planetOrbit";

const DIONE_RADIUS = 0.07;
...
function subtleNoise(x: number, y: number, z: number): number {
    const lowFreq = pseudoNoise3D(x * 3, y * 3, z * 3, 32.0) * 0.7;
    const medFreq = pseudoNoise3D(x * 6, y * 6, z * 6, 33.0) * 0.3;
...
export default function Dione() {
    ...
    const dioneTexture = useLoader(THREE.TextureLoader, "/textures/Saturn - Dione.jpg");
    ...
    const dioneGeometry = useMemo(() => {
        const baseGeometry = new THREE.IcosahedronGeometry(DIONE_RADIUS, 5);
        ...
        const displacementAmplitude = DIONE_RADIUS * 0.02;
    ...
    useFrame(() => {
        ...
        const dioneWorldPos = getDioneWorldPosition(currentDate, saturnPosition);

        if (orbitRef.current) {
            orbitRef.current.position.copy(dioneWorldPos);
        }

        // Tidal locking rotation (Dione always faces Saturn)
        if (dioneRef.current) {
            const dioneRelativePos = new THREE.Vector3().subVectors(
                dioneWorldPos,
                saturnPosition
            );
            const rotationAngle = Math.atan2(dioneRelativePos.x, dioneRelativePos.z);
            dioneRef.current.rotation.y = rotationAngle;
        }
    ...
    return (
        <group ref={orbitRef}>
            <group ref={groupRef}>
                ...
                >
                    <primitive object={dioneGeometry} attach="geometry" />
                    <primitive object={dioneMaterial} attach="material" />
                </mesh>
            </group>
        </group>
    );
