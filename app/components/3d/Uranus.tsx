...
const URANUS_ROTATION_PERIOD_SECONDS = 17.2 * 60 * 60;
const URANUS_RADIUS = 0.75;
...
export default function Uranus() {
    ...
    useEffect(() => {
        if (tiltRef.current) {
            tiltRef.current.rotation.x = URANUS_AXIAL_TILT_RADIANS;
        }
    }, []);
    ...
    useFrame(() => {
        ...
        if (orbitRef.current) {
            const pos = getPlanetOrbitPosition(currentDate, URANUS_ORBIT_PARAMS);
            orbitRef.current.position.copy(pos);
        }

        if (groupRef.current) {
            groupRef.current.rotation.y =
                (t / URANUS_ROTATION_PERIOD_SECONDS) * Math.PI * 2;
        }
        ...
        <group ref={orbitRef}>
            <group ref={tiltRef}>
                <group ref={groupRef} scale={[baseScale, baseScale, baseScale]}>
                    ...
                    >
                        <sphereGeometry args={[URANUS_RADIUS, 128, 128]} />
